...
class Node:
    def __init__(self, key: K, color: bool = True) -> None:
        self.key: K = key
...
class TreeSet(Generic[K]):
    def __init__(self, cmp: Callable[[K, K], bool] = operator.lt) -> None:
    ...
    def count(self, key: K) -> Optional[Node]:
        current = self.root
        while current is not None:
            if key == current.key:
                return current
